...
**The app's name is "Kentucky Aerial Perspective Viewer"** (chosen by the user; it is the page title, the panel heading and the README title). The GitHub repository is still `aerial-perspective-viewer` and the directory `oblique-viewer`, and internal names were left alone on purpose (the npm packages `oblique-viewer-web` and `oblique-viewer-api`, the Docker Compose project and container names, and the API's database `application_name`).
...
npm test           # 78 unit tests: the wording logic, the COG reader, the photo cache, the camera model, the projection, the placement geometry and the thumbnail sizing (node:test, no browser)
```
