...
npm test           # 57 unit tests: the wording logic, the COG reader, the photo cache, the camera model, the projection, the placement geometry and the thumbnail sizing (node:test, no browser)
```
...
- **A TiTiler for the thumbnails was suggested by the user, and stock TiTiler cannot read these photos.** Tried once, locally, with `ghcr.io/developmentseed/titiler:latest` (TiTiler 2.3.0, rasterio 1.5.1) in Docker (the container and image were removed afterwards): `GET /cog/preview.jpg?url=<photo>&max_size=200` on `Bwd_7025_44168.tif` answered **HTTP 500**, with `AttributeError: 'NoneType' object has no attribute 'to_authority'` from `rio_tiler/utils.py` `CRS_to_uri`, because the photo has no coordinate system. So a TiTiler for these photos needs a small custom reader or a georeferencing shim; a stock hosted TiTiler would fail the same way unless it was set up differently (the user's hosted instance was not identified: no address has been given, and none of the URLs given so far is a TiTiler). The user also asked that the app **ping the TiTiler once when it first opens, so it wakes up** (it is expected to sleep between uses), to be removed later; that is **not built**, because there is no address to ping. When one is given, make it a configuration value (for example `VITE_TITILER_URL`), send one cheap request (`/healthz`) with errors ignored, and fall back to the browser-side thumbnails when it is unset or fails. A server-side preview would cut a thumbnail from 60 to 550 KB to roughly 10 KB and could be cached by URL, but it cannot crop around the clicked point (TiTiler crops by geographic bounds).
