...
cd web
npm install
npm run dev        # http://localhost:5173; /api is proxied to the API on 127.0.0.1:3001 (start it first: cd api && npm start)
...
Source layout: `src/main.ts` wires the map to the panel, `api.ts` is the API client, `describe.ts` turns a ranked frame into words (the tested part), `panel.ts` draws the panel, `footprint.ts` owns the map layers for the chosen frame, and `config.ts` holds the basemap settings.
...
- Built: the DuckDB normalization, the PostGIS schema and loader, the query layer with its selection rule (`pipeline/`, `docker-compose.yml`), the API in `api/`, and the map in `web/` that looks up and lists the photos covering a click and draws the chosen footprint. Not built: an automated browser test (the check was a scratch script), any hosting or migration, showing the photos themselves, and projecting the point into a photo.
