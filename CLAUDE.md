...
- Built: the DuckDB normalization, the PostGIS schema and loader, the query layer with its selection rule (`pipeline/`, `docker-compose.yml`), the API in `api/`, and the map in `web/` that looks up and lists the photos covering a click and draws the chosen footprint. The photo pane shows the selected photo, using the COG reader in `web/src/cog.ts`. Not built: an automated browser test (the checks were scratch scripts), any hosting or migration, draping the photo on the map, and marking the point on the photo.
