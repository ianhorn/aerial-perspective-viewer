...
**The basemap is coarser than native.** Checked at Louisville: the basemap (Commonwealth Map) has tiles at levels 19 and 20 but not 21 or 22 (404), so its best detail is 1:443 and at 1:256 it is already enlarged about 1.7× (the car in the screenshot at native scale is soft). **The Phase 3 orthoimagery service (`Ky_Imagery_Phase3_3IN_WGS84WM`) has tiles at level 21 (1:222), which is finer than native**, and 404 at 22. So using it as the basemap at close zoom would make the map sharp up to native; not done (the basemap was chosen by the user "to start out").
